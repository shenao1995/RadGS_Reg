...
def readNAFInfo(path, eval):
    ...
    #     "DSO": data["DSO"],
    #     "nVoxel": data["nVoxel"],
    #     "dVoxel": (np.array(data["dVoxel"])).tolist(),
    #     "sVoxel": (np.array(data["nVoxel"]) * np.array(data["dVoxel"])).tolist(),
    #     "nDetector": data["nDetector"],
    #     "dDetector": (np.array(data["dDetector"])).tolist(),
    #     "sDetector": (
    #             np.array(data["nDetector"]) * np.array(data["dDetector"])
    #     ).tolist(),
    #     "offOrigin": (np.array(data["offOrigin"])).tolist(),
    #     "offDetector": (np.array(data["offDetector"])).tolist(),
    #     "totalAngle": data["totalAngle"],
    #     "startAngle": data["startAngle"],
    #     "accuracy": data["accuracy"],
    #     "mode": data["mode"],
    ...
    for split in splits:
        ...
        # for i_split in random.sample(range(n_split), 10):  # 2 views
        # for i_split in [0, 25]:
            # sys.stdout.write("\r")
            # sys.stdout.write(f"Reading camera {i_split + 1}/{n_split} for {split}")
            # sys.stdout.flush()
