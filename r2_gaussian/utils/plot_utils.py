...
def projs_to_gif(projs_array, file_name):
    colored_images = []
    for i in range(projs_array.shape[-1]):
        # 应用 viridis colormap
        normalized_img = cv2.normalize(projs_array[:, :, i], None, alpha=0, beta=1,
                                       norm_type=cv2.NORM_MINMAX,
                                       dtype=cv2.CV_32F)
        # normalized_img = (normalized_img * 255).astype(np.uint8)
        cmap = plt.get_cmap('viridis')
        rgba_image = cmap(normalized_img)  # 返回 RGBA 图像
        # rgba_image[normalized_img == 0] = [1, 1, 1, 1]  # 设置背景为白色 (R=1, G=1, B=1)
        colored_image = (rgba_image[:, :, :3] * 255).astype(np.uint8)  # 转换为 RGB
        colored_images.append(colored_image)

    # 生成GIF文件
    # output_file = 'object_rotation_viridis.gif'  # 保存文件的路径和名称
    imageio.mimsave(file_name, colored_images, duration=0.01, loop=0)


def volume_show(input_volume, save_path):
    if torch.is_tensor(input_volume):
        input_volume = t2a(input_volume)
    ...
    half_size = input_volume.shape[0] // 2
    input_volume[:half_size, :, :] = 0  # Set half to zero to show the inner structure
    clim = [0.0, 1.0]

    plotter = pv.Plotter(window_size=window_size, line_smoothing=True, off_screen=True)
    plotter.add_volume(input_volume, cmap=colormap, opacity="linear", clim=clim)
